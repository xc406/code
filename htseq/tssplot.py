...
def main(argv):
    ...
    bamfile = HTSeq.BAM_Reader( infile_bam )#"http://hgdownload-test.cse.ucsc.edu/goldenPath/hg19/encodeDCC/wgEncodeOpenChromFaire/wgEncodeOpenChromFaireK562AlnRep1.bam" )#bamfname )#"wgEncodeUwDnaseTh17AlnRep1.bam" 
       ...
       if feature.type == "exon" and feature.attr["exon_number"] == "1":
	      ...
	      if 'chr' in p.chrom: 
                  if p.pos < halfwinwidth:
                      window = HTSeq.GenomicInterval( p.chrom, p.pos - p.pos, p.pos + halfwinwidth, ".")
                  else:
    ...
    for almnt in bamfile:
       if almnt.aligned:
	      ...
	      for step_iv, step_set in tsspos[ almnt.iv ].steps():
	          ...
	          print tsspos[ almnt.iv ], '<-- tsspos[alignment]' 
...
#f.close()
    numpy.savetxt(os.path.join(bampath, bamfname + '1plot.txt'), profile, delimiter = '\t')
