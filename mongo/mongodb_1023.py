...
def updateFOS(path, db, motifChrom="chr17", method="NSD", flankWin=35):
...
#		"motif_genomic_regions_info.chr": motifChrom})
	for test in cursor:
	    ...
	    if fos > 0.95 and count-countCent > 20:#(flankR-flankL-(motifEnd-motifStart)):
...
def updateMotifGenomicRegions(path,db,chromNum,mcollection=None):
    ...
    for infile in glob.glob(os.path.join(path,"hg19_"+chromNum+"_1e3_100_*_fimoout_092914_sort.txt.gz")):
        ...
        tfName = fimofilename.split('hg19_chrom17_1e3_100_')[-1].split('fimoout')[0].split('_')[0]
...
#		},	
    ...
    #updateMotifGenomicRegions(path, db)
    #updateCount(path,db)
    #updateGC(path,db)
